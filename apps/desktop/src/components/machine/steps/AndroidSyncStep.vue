...
// The same bounded, secret-filtered snapshot the macOS guest gets, streamed into the toolchain
    ...
    <StepPanel :step="step">
        ...
        <div class="space-y-3">
            <p class="text-xs leading-5 text-zinc-600 dark:text-zinc-300">
                A bounded, checksummed snapshot of the approved folder is streamed into the
                container and installed atomically. Git metadata,
                <span class="font-mono">node_modules</span>, Gradle and build output, every
                <span class="font-mono">.env</span> file, keys, and certificates are excluded.
                Synchronize again after every source change; it resets the debug-build state.
            </p>
            ...
            <Field
                ...
                :hint="
                    attachedEnvSet
                        ? `${attachedEnvSet.name} is written into the container as .env.production.local at every sync and exported to the build shell. Synchronize again after changing it.`
