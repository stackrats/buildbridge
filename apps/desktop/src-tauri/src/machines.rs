...
impl MachinePaths {
    pub fn resolve(app: &AppHandle, machine_id: &str) -> Result<Self, String> {
...
pub(crate) fn load_registry(app: &AppHandle) -> Result<MachineRegistry, String> {
...
pub(crate) fn save_registry(app: &AppHandle, registry: &MachineRegistry) -> Result<(), String> {
    let path = registry_path(app)?;
    let parent = path
        .parent()
...
fn migrate_legacy_builder(app: &AppHandle) -> Result<MachineRegistry, String> {
    let legacy_path = app
        .path()
        .app_config_dir()
        .map(|path| path.join(LEGACY_CONFIG_FILE))
        .map_err(|error| error.to_string())?;
