...
| The live reload PR's macOS CI job failed one `buildbridge-machines` test: the debug job that must refuse an environment file overwriting `BUILDBRIDGE_LIVE_RELOAD_URL` reported success on a Mac and failure on Linux | The guest job wrapper runs its body in a subshell under `set -eu` with an `EXIT` trap that writes `$?` to the job's status file. bash 3.2, which is `/bin/sh` on every Mac and so on every macOS guest, ends the shell on an assignment to a readonly variable or an unbound-variable expansion as POSIX asks, but runs the trap with `$?` of 0, so the job recorded 0 and the wrapper exited 0. Reproduced in the `bash:3.2` image; bash 5 and dash hand the trap the real status. **A product fault beyond the test**: a smoke build or iPhone run on the guest that died this way would have been reported as a success with its work undone, since only ordinary command failures kept their status | The wrapper trusts a 0 only once the body ran to its end: the subshell sets a `job_done` flag as its last statement and the trap turns a 0 without that flag into a 1. Every deliberate early exit in a job body is already nonzero, so nothing that succeeds is misreported. Verified under bash 3.2 for a readonly assignment, an unbound variable, a failing command, an explicit `exit 3` and a clean body | Fixed 2026-09-17 |
