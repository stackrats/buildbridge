...
| The Debug build was refused because its bundle identifier had no profile | A project's Debug configuration often carries its own suffixed identifier, and Apple profiles are per App ID | Read the identifier the Debug build carries from the guest, register it at Apple with the main App ID's capabilities copied, and make the development profile for it | Resolved; the debug build keeps its own identifier |
| An iPhone Debug run used the production identifier despite the project's separate Debug App ID; affiliation deep links then failed to open | Device readiness accepted any development profile listing the phone, the old workspace had no resolved Debug identifier, and the build renamed the app to fit the production profile. The retained logs confirm failed URL openings but do not identify which installation iOS rejected | Require the resolved Debug identifier, matching profile, development certificate and phone in readiness; resolve the identifier again before each run; reuse or create the matching profile through Prepare signing; refuse identifier substitution and reattachment to a run under another identifier. Device refresh resolves unknown identifiers without requiring an Apple API key, preserving imported-profile workflows | Code fix; physical-device confirmation pending |
...
| The macOS half of CI had never passed: `cargo test --workspace` exited 101 on every run since the workflow was added, while Linux passed each time | `process_alive` in `ops.rs` answered from `/proc`, and returned a bare `true` on every other platform. So on a Mac every pid looked alive, a dead owner's `operation.lock` was never replaced, and the lock test's "a dead owner does not hold it" could not hold. The live half of that test was already guarded to Linux, so the divergence was known when it was written; the dead half was not. This is a product fault and not only a test one — on a Mac a lock left behind by a crashed buildbridge would hold its machine for good, which is the opposite of what the type's own comment promises | `process_alive` keeps the `/proc` answer on Linux, where it costs no process, and asks `ps -p` elsewhere. `ps` reports a process whoever owns it, where signal 0 refuses on another user's live process and would report it as gone — and a lock wrongly called dead is the dangerous direction, since it hands one guest to two processes. The test's live case now spawns a child of its own rather than assuming pid 1, so it runs on every Unix instead of Linux alone | Fixed 2026-09-08, after v0.1.0 was tagged; found by reading the code, since the run logs need repository admin and this token has none |
| With the lock fixed, the macOS half of CI still failed, in `google_play`'s upload test | Its fake Play API binds a listener and sets it non-blocking so the accept loop can poll a stop flag. **macOS and the BSDs give an accepted socket the listener's non-blocking flag; Linux always hands back a blocking one.** So on a Mac the very first `read_line` returned `WouldBlock` before the client had sent anything, the server thread panicked, the upload failed with a connection error instead of the hash mismatch the test asserts, and `Drop` then panicked again joining the dead worker — three panics from one platform difference. The 5-second read timeout the fixture sets was dead code there too, since it does not apply to a non-blocking socket | The accept loop sets each accepted stream back to blocking. `sharing_transport_tests` builds its fake service the same way and had the same latent bug, so both were fixed together | Fixed 2026-09-08. Found by pushing a throwaway branch whose workflow echoes the failing test through `::error::`, since run logs need repository admin but annotations are public |
| Six `android_device` tests failed on macOS, five of them with `PoisonError` | Only one was real: `install_and_launch_…` cancels the run after a fixed 600 ms to stand in for a Stop from the desktop, which is comfortable on this host and not on a slower runner, where the cancellation landed before the launch was confirmed and failed the run instead of stopping it. That panic happened while holding `TEST_DEVICE`, the mutex that serialises these tests, so the five that followed died on the poisoned lock and hid the one that mattered | The fixture waits for the log session to actually start — the fake `adb` records its calls, so the cancelling thread watches for the `logcat` one — instead of guessing a delay, which removes the race on any machine. Taking the lock recovers from poisoning as well: it serialises tests and guards no state a panic could have corrupted, so one failure no longer costs five diagnoses | Fixed 2026-09-08 |
| `transporter`'s guest timeout test failed on macOS, reporting a host-side bridge failure instead of the upload timing out | **A product fault, and the worst of the set: the guest helper's upload deadline had never worked, on the only platform it ever runs on.** Python installs signal handlers with `SA_RESTART`, and the BSDs honour it for `select()`: the kernel restarts the wait rather than failing it with `EINTR`, so the handler does not run until the syscall returns on its own. `signal.alarm(UPLOAD_TIMEOUT_SECONDS)` therefore did nothing on a macOS guest — which is every guest — and a Stop waited on a transporter that had stopped answering. Linux fails `select()` with `EINTR` regardless, which is why only CI's macOS half ever showed it | `transporter.py` calls `signal.siginterrupt(signum, True)` for each handler it installs, so the wait is interrupted rather than restarted | Fixed 2026-09-08; found only because the release pass looked at CI rather than the local suite |
...
| The live reload PR's macOS CI job failed one `buildbridge-machines` test: the debug job that must refuse an environment file overwriting `BUILDBRIDGE_LIVE_RELOAD_URL` reported success on a Mac and failure on Linux | The guest job wrapper runs its body in a subshell under `set -eu` with an `EXIT` trap that writes `$?` to the job's status file. bash 3.2, `/bin/sh` on every Mac, ends the shell on an assignment to a readonly variable or an unbound-variable expansion as POSIX asks, but runs the trap with `$?` of 0, so the job recorded 0 and the wrapper exited 0. Reproduced in the `bash:3.2` image; bash 5 and dash hand the trap the real status. Corrected the same day: the script is the ssh remote command, so the guest's *login shell* runs it, zsh on any macOS since Catalina, and zsh skips the trap on those errors altogether (see the row below); the bash 3.2 guard stays for a guest whose login shell is bash. **A product fault beyond the test**: a smoke build or iPhone run on the guest that died this way would have been reported as a success with its work undone, since only ordinary command failures kept their status | The wrapper trusts a 0 only once the body ran to its end: the subshell sets a `job_done` flag as its last statement and the trap turns a 0 without that flag into a 1. Every deliberate early exit in a job body is already nonzero, so nothing that succeeds is misreported. Verified under bash 3.2 for a readonly assignment, an unbound variable, a failing command, an explicit `exit 3` and a clean body | Fixed 2026-09-17 |
| On a host that had run an earlier release, the iPhone step warned that the udev rule "differs from the expected content", all six checks showed as failed, and the **Reinstall the host USB rule** button was disabled | Two faults. The rule check compared the whole file byte for byte, and the lowercase-brand commit had changed the comment header from "Written by BuildBridge" to "Written by buildbridge"; the rule line itself was identical, so passthrough worked while the step said it did not. And the step disabled its primary action whenever the step was pending, whichever rung it stood on, so a host waiting on its test build could not reinstall a udev rule that no build depends on | The check compares the lines udev acts on and ignores comments and blank lines, so a header change never asks a working host to reinstall, while a changed or added rule line still does. The pending gate now withholds only the signing and run rungs, which the engine refuses without the test build anyway, and the withheld button names that reason; every host and phone rung acts at any time | Fixed 2026-09-17 |
| The unsigned test build failed in the guest at Resolving CocoaPods: "could not find compatible versions for pod IONCameraLib", with the lock pinning 1.0.4 and the project's `@capacitor/camera` 8.2.4 wanting 2.x | The host's `@capacitor/camera` had been upgraded, and its pod now depends on a newer IONCameraLib than the committed `ios/App/Podfile.lock` pins. `pod install` keeps every pinned version, so it cannot satisfy both and stops. The lock-drift flow the design describes, a refreshed guest lock that the step offers for adoption, was never reached, because it only fires when `pod install` itself rewrites the lock; a conflicting lock makes it fail instead. A Linux host cannot run CocoaPods, so `cap sync ios` there copies assets and updates the plugin list and can do nothing about the lock | The guest recipe runs `pod install`, and on failure reads the pods CocoaPods names in its own "could not find compatible versions" lines, limited to the characters a pod name can hold, and retries with `pod update` for exactly those, as CocoaPods advises. The lock then differs from the committed one, the existing marker reports it, and the test build step offers the refreshed lock for adoption. Any other failure still fails the build as before, and a project with no committed lock is left to plain `pod install`. Verified with a fixture `pod` on Linux and under bash 3.2. The native Mac path keeps its `--deployment` install: that host has CocoaPods, and the path refuses by design to alter the host project | Fixed 2026-09-17 |
| The macOS half of CI failed one `buildbridge-machines` test with `File exists` from the Android HTTP fixture's `create_dir`, on a change that had not touched it | Every test fixture named its temp directory by the process id and a nanosecond timestamp. A macOS clock ticks in microseconds, and a test binary creates fixtures on many threads at once, so two fixtures born in the same instant took the same name and the second died. Linux clocks are fine-grained enough that it had never shown there | `test_scripts::fixture_dir` names a directory by the process id and a process-wide counter and creates it; every test fixture in the crate uses it, and none names a directory by a clock any more. Production temp names in `archive`, `android_signing` and `android_device` still use a timestamp, but each is created once per operation rather than in parallel by a test runner | Fixed 2026-09-17 |
| Release CI intermittently failed while executing the askpass fixture or waiting for mock Transporter startup | Parallel test process creation can briefly keep an executable open for writing; the Transporter cancellation test also allowed only five seconds for its subprocess chain to become ready | The askpass assertion uses the fixture helper’s bounded retry for `ExecutableFileBusy` without changing its script. Transporter readiness has a bounded twenty-second budget, detects early worker exit, and always cancels and joins before reporting startup diagnostics; production behavior is unchanged | Fixed 2026-09-22 |
| With the CocoaPods retry shipped and installed, the same IONCameraLib failure came back unchanged | The retry never ran: the failure was earlier. Capacitor's `cap sync ios` rewrites the Podfile's plugin block and then runs `pod install` itself, in the syncing phase before buildbridge's Resolving CocoaPods phase, and it has no switch to leave that out, only `CAPACITOR_COCOAPODS_PATH` to choose which `pod` it runs. Found by reproducing in the guest over buildbridge's own SSH identity: `pod update IONCameraLib` there resolved 2.0.0 at once, so the retry was sound and simply unreached | The sync runs with `CAPACITOR_COCOAPODS_PATH=/usr/bin/true`, so Capacitor rewrites the Podfile and cleans the project but leaves CocoaPods to the next phase, where the retry lives. Verified on the guest against the project with its stale lock restored: sync passes, install refuses the pod, the update resolves it, and the lock-changed marker fires | Fixed 2026-09-17 |
| The macOS half of CI failed `android_device`'s install-and-launch test with no console lines streamed, on a change to the iOS recipe | The test's stand-in for a Stop from the desktop fired as soon as the fake `adb` had been called with `logcat`, not once the run had read the two lines the fake prints, so on a slow runner the run ended before they arrived. The 2026-09-08 fix to this test had moved the Stop from a fixed delay to that call, which was later but still not late enough | The Stop waits until the run's progress has carried both lines, with the same 30-second ceiling, so it lands where a person's would: after the console is visibly streaming | Fixed 2026-09-17 |
| With Capacitor's sync kept out of CocoaPods, the retry ran and failed: "The `IONCameraLib ` Pod is not installed and cannot be updated", with a space inside the name | The guest job script is passed as the ssh remote command, so the guest's login shell runs it, and on macOS that is zsh. The retry passed the pod names as an unquoted parameter and relied on sh's word splitting; zsh does not split, so `pod update` got one argument holding the name and a trailing space. Every earlier verification had run the rendered script under `/bin/sh` on purpose, which hid it | The names are built into positional parameters one line at a time and passed as `"$@"`, which every shell treats alike. The recipe's fixture test now runs the script under each of `/bin/sh`, `/bin/bash` and `/bin/zsh` the host has, so the macOS CI runner covers zsh. Verified as the ssh remote command on the guest: install refuses the pod, the update resolves 2.0.0, the marker fires | Fixed 2026-09-17 |
| Running the job wrapper under the guest's zsh for a body with a readonly assignment hung forever: no status file, worker gone | **A product fault, and the worst of today's set.** zsh ends the worker subshell on an assignment or expansion error without running its EXIT trap at all, so no status is ever written, and the wrapper's reader waited only for the status file. A smoke build, device run or Android job that died this way held its step until someone stopped it. bash 3.2 ran the trap with a wrong status; zsh does not run it | The reader also watches the worker's pid. Once the worker is gone, a status it wrote is already in place, since the trap runs before exit; none means the shell skipped the trap or the worker was killed, and the reader records 1 with a line saying the job ended without reporting. A test kills the worker outright, which no shell's trap survives. Verified as the ssh remote command on the guest for a readonly assignment, an unbound variable, a killed worker, a failing command, an explicit exit and a clean body | Fixed 2026-09-17 |
...
**Stop all machines** is available in the desktop toolbar and beside a memory-shortage error.
It stops only the machines in BuildBridge's registry, concurrently, using their normal shutdown
path. Active operations are cancelled first and given a bounded wait to release their locks;
disk migrations and operations owned by another process remain protected and return individual
failures. The result lists stopped, already stopped and failed machines without deleting their
containers, disks or saved data. New desktop operations are held off until the batch and its
status refresh finish. A stopped machine can still show its earlier start failure: stopping it
does not resolve insufficient host memory. Other applications and unrelated VMs are unaffected,
so they may need to be closed separately before a memory-constrained machine can start. The
shared prerequisite error names the machine rather than Docker-OSX, and stopped machines do not
show an uptime from their previous run.
...
1. **Prepare the host** installs one udev rule, `/etc/udev/rules.d/40-buildbridge-iphone.rules`,
   ...
   and offers the mirror removal. The user replugs the phone so the new mode applies. The
   installed file is judged by the lines udev acts on: a header written by an earlier release,
   or a note someone added, is not a modification, and only a changed or added rule line asks
   for a reinstall. Preparing the host and the phone never waited for the test build, so the
   step's rungs up to Developer Mode act while the step is pending; only signing and the run
   itself wait, and their button says what for.
...
6. **Build and run** compiles the App scheme in `Debug` for `generic/platform=iOS` inside the same
   ...
   the development profile for that identifier. Device readiness checks the resolved identifier,
   profile, development certificate and phone together. Device refresh resolves an unknown
   Debug identifier without an Apple API key, so imported profiles remain usable. Each run
   resolves the identifier again and requires a matching profile; it never renames the Debug
   app to fit a production profile. When Debug has its own identifier it installs beside the
   store build. The build verifies the bundle (signature, embedded profile UUID,
...
Instruments, and a QR/OTA install route. Capacitor live reload is supported in **Build and run**
over a development server the iPhone can reach directly, as described below.
...
Android and iPhone Capacitor projects can opt into **Live reload** beside **Build and run** (2026-09-17). The user starts the project's development server separately and supplies its HTTP or HTTPS URL. Buildbridge checks that the server responds before building or installing, then builds and installs a debug shell whose WebView loads that server; subsequent web edits use the server's own reload or HMR support without another native build. Native code, plugins and native configuration still require synchronizing the updated source and building again; Android's guided **Build and run** synchronizes automatically, while the iPhone run uses the prepared snapshot. The development server controls its own environment and must remain running; the environment selected for the native build does not configure that external process. Live reload defaults off and its draft resets when the approved project changes. Other frameworks do not offer this option.

For an Android localhost URL, the device runner reverses the URL's TCP port to the same host port with ADB before launch. HTTP traffic and HMR WebSockets share that route when the development server serves both on that port; a separately configured HMR port needs its own reachable route. The session owns only mappings it creates, preserves an existing matching mapping, and refuses to replace a conflicting one. On a normal session end, cancellation or failure, it attempts to remove its own mapping; a disconnected device can prevent cleanup. The app stays installed, but removing that route stops its access to the development server. LAN URLs instead require the phone to reach the server directly and the server to listen on a reachable interface. This option does not start the development server or forward unrelated API ports.

The Android live reload URL is applied through temporary debug Gradle inputs, together with the HTTP debug allowance when the URL needs it. The overlay preserves the rest of Capacitor's configuration and never writes the override into the host project or the synchronized source. Release builds, their preparatory debug builds and remote release jobs do not enable it. Each retained debug APK records the URL it was built with, so installing that APK later restores the required ADB route from the artifact's metadata rather than the current draft. Turning the option off takes effect after rebuilding and installing a normal debug APK. See [Capacitor's live reload workflow](https://capacitorjs.com/docs/guides/live-reload).

An iPhone uses a reachable LAN address or hostname; localhost refers to the phone and is rejected. A development server on this computer must listen on a network interface the phone can reach, normally with both on the same Wi-Fi. USB passthrough installs the app and carries its console; it does not forward the development-server port. The server's HMR WebSocket must also be reachable from the phone. **Stop** ends the console session and leaves the installed app's direct server connection available while the server is running. The host's server preflight cannot prove reachability from the phone.

For iPhone live reload, the guest makes a separate copy of the compiled Debug `.app` and changes only that copy's Capacitor `server.url` and the necessary `Info.plist` entries. An HTTP URL gets an exception for its exact host; HTTPS retains the project's transport policy. The copy keeps existing settings, supplies a local-network permission explanation when one is missing, and adds the server host to an existing app-bound domain list when space permits. The fixed signing helper signs the outer app again with the same development identity and preserved entitlements, then the existing signature, profile, bundle identifier and version checks run before installation. The host project, synchronized source and reusable Xcode product remain unchanged even if copying or signing is interrupted. Release builds never invoke this overlay. See Apple's [domain-specific transport exceptions](https://developer.apple.com/documentation/BundleResources/Information-Property-List/NSAppTransportSecurity/NSExceptionDomains), [local network privacy](https://developer.apple.com/documentation/technotes/tn3179-understanding-local-network-privacy), and [code-signature format](https://developer.apple.com/documentation/xcode/using-the-latest-code-signature-format).

The retained iPhone run records its live reload URL, and reconnecting to a detached session requires the same phone and live reload settings. An older run without the field is treated as a bundled app. Turning live reload off requires building and installing another Debug app. Fixture tests cover the copied app's configuration, scoped permissions, unchanged originals, malformed inputs and reconnect mismatches. A host C fixture exercises the signing helper's fixed arguments and keychain/password cleanup through success and failure; it does not validate the macOS SDK or actual code signing. iPhone live reload still needs acceptance on a macOS guest and physical phone.
...
The override reaches only as far as the network does. When devices are listed, each ready phone is asked for its Wi-Fi or Ethernet address and the device step warns when that address shares no network with this computer's, naming the network to join: an app calling an API served on this computer over its LAN address gets no response from a phone on mobile data or another Wi-Fi, whatever the build allows. A live reload session's ADB reverse mapping carries its development-server port independently of Wi-Fi; it does not also forward API ports. Emulators are exempt, and a phone that cannot answer is not warned about.
