...
pub fn probe_host_for(provider: MachineProvider) -> HostPrerequisites {
    ...
    // The toolchain container runs macOS-free on any Docker; the macOS providers need the
    // Linux host's KVM, and one of them its X display, the other its tun device.
    let macos = provider.is_macos();
    let supported_host = if macos { supported_host } else { cfg!(unix) };
    if !supported_host {
        issues.push(if macos {
            format!(
            ...
            format!(
                "{} currently requires a Unix host with Docker.",
                provider.label()
            )
        });
    }
    if !docker_cli {
        issues.push(if macos {
            "Install the Docker CLI before creating a macOS builder.".to_string()
        } else {
            "Install the Docker CLI before creating an Android builder.".to_string()
        });
    ...
    F: FnMut(LaunchProgress),
{
    ...
    match config.provider {
        ...
        MachineProvider::AndroidToolchain => {
            return android::launch(container_name, config, options.disk_dir, on_progress);
        }
...
pub(crate) fn ensure_image(image: &str) -> Result<(), ProviderError> {
    ...
    if !exists {
        run_docker("image pull", &["pull".to_string(), image.to_string()])?;
    }
...
/// A throwaway container's view of a directory it is asked to empty.
const THROWAWAY_STORAGE_DIR: &str = "/buildbridge-storage";

/// Empties a directory whose files an image created as root, which this user cannot unlink
/// from outside the container: a throwaway container of that image with the directory bound
/// deletes them with a fixed argv, and the directory itself is then this user's to remove.
pub(crate) fn empty_directory_with_container(image: &str, dir: &Path) -> Result<(), ProviderError> {
    validate_bind_path(dir, "storage directory")?;
    let args = vec![
        "run".to_string(),
        "--rm".to_string(),
        format!("--volume={}:{THROWAWAY_STORAGE_DIR}:rw", dir.display()),
