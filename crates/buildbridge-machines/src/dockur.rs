...
pub(crate) fn ensure_memory_available(config: &MachineConfig) -> Result<(), ProviderError> {
...
/// unlink from outside the container.
pub(crate) fn empty_storage_with_container(storage_dir: &Path) -> Result<(), ProviderError> {
    empty_directory_with_container(DOCKUR_IMAGE, storage_dir)
}
...
mod tests {
    ...
    fn the_screen_is_served_on_the_port_after_ssh_only_for_this_provider() {
        ...
        assert_eq!(MachineConfig::default().display_port(), None);
