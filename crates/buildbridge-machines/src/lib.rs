...
mod signing;
#[cfg(all(test, unix))]
mod signing_device_resign_tests;
mod smoke_build;
mod ssh;
mod templates;
#[cfg(test)]
...
pub fn container_name(machine_id: &str) -> String {
    container_name_for(MachineProvider::DockerOsx, machine_id)
}

/// A machine's container name: the macOS providers share one naming, a toolchain container
/// says what it is.
pub fn container_name_for(provider: MachineProvider, machine_id: &str) -> String {
    match provider {
        MachineProvider::DockerOsx | MachineProvider::DockurMacos => {
...
pub enum MachineProvider {
    ...
    DockurMacos,
    /// A pinned JDK image kept alive with `sleep`, its home bound from this host, with the
    /// Android SDK, Gradle and Node prepared inside it; builds run through `docker exec`.
    AndroidToolchain,
}
...
impl MachineProvider {
    pub fn image(self) -> &'static str {
        match self {
            ...
            Self::AndroidToolchain => ANDROID_IMAGE,
        }
    }
    ...
    pub fn label(self) -> &'static str {
        match self {
            ...
            Self::AndroidToolchain => "Android toolchain",
        }
    }

    pub fn platform(self) -> MachinePlatform {
        match self {
            Self::DockerOsx | Self::DockurMacos => MachinePlatform::Ios,
            Self::AndroidToolchain => MachinePlatform::Android,
        }
    }

    /// Whether the machine is a macOS guest reached over SSH, with a disk, a screen and a
    /// QEMU control socket; the toolchain container has none of those.
    pub fn is_macos(self) -> bool {
        self.platform() == MachinePlatform::Ios
    }
}
...
impl MachineConfig {
    pub fn validate(&self) -> Result<(), ProviderError> {
        ...
        if !(1..=80).contains(&name_length) {
            return Err(ProviderError::InvalidConfig(
                "the machine name must contain between 1 and 80 characters".to_string(),
            ));
        }
    ...
    pub fn display_port(&self) -> Option<u16> {
        match self.provider {
            MachineProvider::DockerOsx | MachineProvider::AndroidToolchain => None,
    ...
    /// Every host port the machine's container publishes. A toolchain container publishes
    /// none: nothing in it listens.
    pub fn published_ports(&self) -> Vec<u16> {
        if !self.provider.is_macos() {
            return Vec::new();
        }
...
pub enum ProviderError {
    ...
    #[error("Android toolchain failed: {0}")]
    AndroidToolchain(String),
}
