    ...
    F: FnMut(SigningProvisioningProgress),
{
    ...
    if distribution.is_none() && development.is_none() {
        return Err(ProviderError::GuestBridge(
            "the signing credentials hold no identity: store a distribution identity or a development identity first"
                .to_string(),
        ));
    }
    ...
    on_progress(signing_progress(
        ...
        total_bytes,
        started_at,
        "Validating the signing credentials and preparing the password-safe guest helper.",
    ));
    let prepare = format!(
    ...
    let operation = (|| {
        ...
        for profile in &profiles {
            ...
            let Some(required_fingerprint) = required_fingerprint else {
                return Err(ProviderError::GuestBridge(format!(
                    "provisioning profile {} is a {identity_label} profile, but the credentials hold no {identity_label} identity",
                    profile.uuid
                )));
            };
            if !profile
            ...
            {
                return Err(ProviderError::GuestBridge(format!(
                    "provisioning profile {} does not include the {identity_label} certificate in the credentials",
                    profile.uuid
                )));
            }
        }
...
pub(crate) fn validate_signing_material(
    ...
    for profile_path in profile_paths {
        ...
        total_bytes = total_bytes
            ...
            .ok_or_else(|| {
                ProviderError::GuestBridge("the signing material size is invalid".to_string())
            })?;
