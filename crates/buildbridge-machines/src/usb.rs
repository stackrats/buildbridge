...
impl MachineUsbStatus {
    /// The status of a machine that takes no phone: a toolchain container. Nothing is probed.
    pub fn not_applicable() -> Self {
        Self {
            host: HostUsbStatus {
                supported: false,
                rule: UdevRuleState::Missing,
                rule_path: USB_UDEV_RULE_PATH.to_string(),
                usbmuxd_active: false,
                plugdev_gid: None,
                devices: Vec::new(),
                issues: Vec::new(),
            },
            ...
            container_issue: Some("A toolchain container takes no phone.".to_string()),
            qmp_reachable: false,
            attached: None,
...
impl UsbAttachRoute {
    pub fn for_provider(provider: MachineProvider) -> Self {
        match provider {
            // The toolchain container takes no phone; the engine refuses before this is asked.
            MachineProvider::DockerOsx | MachineProvider::AndroidToolchain => Self::BusPort,
