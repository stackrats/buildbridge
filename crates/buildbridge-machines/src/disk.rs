...
impl DiskLayout {
    pub fn for_config(config: &MachineConfig) -> Self {
        match config.provider {
            // A toolchain container has no disk; its launch never reaches a layout, and a
            // caller that asks anyway gets the plain one.
            MachineProvider::DockerOsx | MachineProvider::AndroidToolchain => Self::DockerOsx,
    ...
    F: FnMut(DiskMigrationProgress),
{
    ...
    if !config.provider.is_macos() {
        return Err(ProviderError::UsbPassthrough(
            "a toolchain container has no macOS disk and takes no phone; there is nothing to migrate".to_string(),
        ));
    }
    ...
    F: FnMut(ContainerRebuildProgress),
{
    ...
    if !config.provider.is_macos() {
        return Err(ProviderError::UsbPassthrough(
            "a toolchain container has no phone controller to rebuild for".to_string(),
        ));
    }
    let disk = MachineDisk::for_launch(options, config)?;
    validate_bind_path(options.qmp_dir, "control socket directory")?;
