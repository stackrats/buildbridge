...
impl QmpEndpoint {
    pub fn for_machine(provider: MachineProvider, qmp_dir: &Path, container_name: &str) -> Self {
        match provider {
            // A toolchain container runs no QEMU; nothing ever listens at this socket, and
            // every caller checks the provider before reaching for it.
            MachineProvider::DockerOsx | MachineProvider::AndroidToolchain => {
                Self::Socket(qmp_dir.join(QMP_SOCKET_NAME))
            }
