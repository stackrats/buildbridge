    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            let lines = forwarder
                ...
                .unwrap_or_default();
            if !lines.is_empty()
                && let Err(error) = handle.block_on(client.append_logs(&build_id, lines))
            {
            ...
            if ticks.is_multiple_of(LEASE_RENEWAL_TICKS)
                && let Err(error) = handle.block_on(client.renew_lease(&build_id))
            {
...
pub(crate) async fn run_remote_apple_archive(
    ...
    let archived = run_apple_signed_archive(app, payload.machine_id.clone(), env_set_id).await?;
