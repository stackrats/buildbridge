...
pub struct StoredMachineTemplate {
    pub(crate) id: String,
...
#[ts(export)]
#[serde(rename_all = "camelCase")]
pub struct MachineTemplateSummary {
    pub(crate) id: String,
    pub(crate) name: String,
    #[ts(type = "number")]
    pub(crate) created_at_epoch_seconds: u64,
...
impl TemplatePaths {
    pub(crate) fn resolve(app: &Engine, template_id: &str) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            data_dir: app.data_dir().join(TEMPLATES_DIRECTORY).join(template_id),
        })
    }
...
fn templates_config_root(app: &Engine) -> Result<PathBuf, String> {
    Ok(app.config_dir().join(TEMPLATES_DIRECTORY))
}
...
fn save_template_record(app: &Engine, template: &StoredMachineTemplate) -> Result<(), String> {
...
pub(crate) fn list_templates(app: &Engine) -> Result<Vec<StoredMachineTemplate>, String> {
...
pub(crate) fn template_dir_for(app: &Engine, machine_id: &str) -> Result<Option<PathBuf>, String> {
...
pub(crate) fn template_name(app: &Engine, template_id: &str) -> Option<String> {
...
pub(crate) fn template_ref_for(app: &Engine, machine_id: &str) -> Option<MachineTemplateRef> {
...
fn summarize_templates(app: &Engine) -> Result<Vec<MachineTemplateSummary>, String> {
...
pub async fn list_machine_templates(app: &Engine) -> Result<Vec<MachineTemplateSummary>, String> {
...
pub async fn save_machine_template(
    app: &Engine,
    machine_id: String,
...
pub async fn delete_machine_template(
    app: &Engine,
    template_id: String,
    input: ConfirmInput,
